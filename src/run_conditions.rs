...
/// Run condition that returns `true` if [`WorldSwapStatus`] just entered
/// [`Background`](WorldSwapStatus::Background).
///
/// Note that this only detects entering the background the first time the world updates, and if the world updated
/// while not in the background. If you use [`BackgroundTickRate::Never`], then this won't detect movement between
/// foreground and background (and the other tick rate options may also not detect it if you swap back and forth
/// too fast).
...
/// Run condition that returns `true` if [`WorldSwapStatus`] just entered
/// [`Foreground`](WorldSwapStatus::Foreground).
///
/// Note that this only detects entering the foreground the first time the world updates, and if the world updated
/// while not in the foreground. If you use [`BackgroundTickRate::Never`], then this won't detect movement between
/// background and foreground (and the other tick rate options may also not detect it if you swap back and forth
/// too fast).
