...
/// in the foreground. [`Self::Join`] to drop the foreground world and put the background world in the foreground.
///
...
pub enum SwapCommand {
    ...
    ///
    /// If there is no background world, then the app will shut down.
    Join,
}
...
pub enum WorldSwapState {
    ...
    ///
    /// Note that background world may not update if [`BackgroundTickRate::Never`] is configured in [`WorldSwapPlugin`].
    Background,
}
...
///
/// The world might be [`Suspended`](WorldSwapState::Suspended) or in the [`Background`](WorldSwapState::Background).
...
impl WorldSwapApp {
    ...
    pub fn new(mut app: App) -> Self {
        if !app.world.contains_resource::<WorldSwapState>() {
